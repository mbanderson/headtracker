...
for i = 1:numel(color_strs)
    figure(h2); hold on;
    g_world = Quaternion.Rbw(Test.qs(i+1,:)')*Test.accel_meas(i,:)';
...
for i = 1:numel(color_strs)
    figure(h3); hold on;
    m_world = Quaternion.Rbw(Test.qs(i+1,:)')*Test.mag_meas(i,:)';
    fprintf('(x,y,z)=(%.2f,%.2f,%.2f)\n',m_world);
