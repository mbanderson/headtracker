...
% w_jc_c = zeros(3,length(tspan));
% w_jc_c_dot = zeros(3,length(tspan));
% 
% amplitude = 0.5;
% w_jc_c(1:2,:) = amplitude*[cos(2*pi/2.5*tspan); sin(2*pi/2.5*tspan)];
% w_jc_c_dot(1:2,:) = 2*pi/2.5*amplitude*[-sin(2*pi/2.5*tspan); cos(2*pi/2.5*tspan)];
...
for i = 1:N
    ...
    if i == 1
        ...
        [w_jc_c_TA(:,i), w_jc_c_CANP(:,i)] = ...
            constelAngVel(W, w_jc_c_dot_est(:,i-1:i), deltaT, w_jc_c_TA(:,i-1));
    end
...
for j = 1:3
    subplot(3,1,j); hold on; grid on
    plot(tspan, r_cw_c_ddot(1,:), 'linew', 2);
    plot(tspan, r_cw_c_ddot_est(1,:), 'r--', 'linew', 2);
...
for j = 1:3
    subplot(3,1,j); hold on; grid on
    plot(tspan, r_cw_w_ddot_est(j,:), 'r--', 'linew', 2);
